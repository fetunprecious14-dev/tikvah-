...
- `ADMIN_ALERT_EMAIL` — where an alert email is sent for every new user message (logged, not sent, if unset)
- `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` — Telegram bot used to alert the admin of every new user message; both must be set together or alerts are logged instead of sent. Create a bot via BotFather to get the token, then message the bot once and look up its `chat_id` via `getUpdates`
- `ADMIN_ALERT_PHONE` — where urgent-flag alert SMS texts are sent, E.164 format e.g. `+15551234567` (logged, not sent, if unset). Unlike email/Telegram, SMS is only sent for messages flagged urgent, since it costs per message
...
- `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` — Web Push credentials for browser/PWA notifications (generate with `npx web-push generate-vapid-keys`); without all three, pushes are logged to stdout instead of sent. `VAPID_SUBJECT` is a `mailto:` address or URL push services may use to contact you
...
- **Crisis handling:** a regex-based heuristic (imperfect by design — flagged as such in the UI) checks every user-authored message for suicidal intent, self-harm, homicide, domestic abuse, panic attack, psychosis, child abuse, or sexual assault language. A match shows crisis resources (988, IASP directory) immediately client-side, marks the conversation `urgent` server-side (sorted first in the admin inbox regardless of filter), and additionally texts the admin if `ADMIN_ALERT_PHONE` is configured.
- **Admin alerts:** the admin is emailed (`ADMIN_ALERT_EMAIL`) and/or Telegram-messaged (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`) for every new user message, not just urgent ones — either or both channels can be configured.
- **Notifications:** in-app (a bell with unread count in the header), email, and optional browser/PWA push — all created when an admin replies. The app is an installable PWA (`manifest.webmanifest`, `sw.js`); the bell menu offers "Get notified here when we reply" when push is supported and `VAPID_PUBLIC_KEY` is configured, subscribing the current device via `/push/subscribe`. Resource/maintenance notification types exist in the schema for future use but nothing currently creates them.
