...
import { recommendResources, type ResourceRecommendation } from '@/lib/recommendations';
...
function Journal() {
  ...
  const save = async () => {
    ...
    const recommendations = recommendResources(emotion.primary, emotion.secondary);
    setDraftEmotion(emotion);
    setSaving(true);
    try {
      const nextEntry = createJournalEntry(text, anonymousId);
      nextEntry.emotion = emotion;
      nextEntry.recommendations = recommendations;
...
function RecommendationList({ recommendations, compact = false }: { recommendations: ResourceRecommendation[]; compact?: boolean }) {
  return <div className={`recommendation-list ${compact ? 'recommendation-list-compact' : ''}`} data-testid="recommendation-list" aria-label="Private resource recommendations">
    <div className="flex items-center justify-between gap-4">
      <span className="text-[10px] font-semibold uppercase tracking-[.16em] text-primary">A few places to begin</span>
      <span className="text-[10px] text-muted-foreground">Up to three</span>
    </div>
    <div className="mt-3 grid gap-2">
      {recommendations.slice(0, 3).map(recommendation => <Link key={recommendation.id} href={recommendation.href} data-testid={`recommendation-${recommendation.id}`} className="group rounded-lg border border-border/70 bg-background/50 p-3 transition hover:border-primary/50">
        <span className="text-[10px] font-semibold uppercase tracking-[.12em] text-primary">{recommendation.type}</span>
        <span className="mt-1 block text-sm font-medium group-hover:text-primary">{recommendation.title}</span>
        {!compact && <span className="mt-1 block text-xs leading-5 text-muted-foreground">{recommendation.description}</span>}
      </Link>)}
    </div>
    {!compact && <p className="mt-3 text-[11px] leading-5 text-muted-foreground">A gentle suggestion, not a prescription. Choose one, or none.</p>}
  </div>;
}
